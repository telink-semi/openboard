...
extern void my_data_broadcast(unsigned char *dat,int len);
...
int led_wink(void *par){
	(void)par;
	static unsigned char sta=0;
	sta = !sta;
	drv_gpio_write(LED_PERMIT, sta);
	return 0;
}

ev_timer_event_t *brc_winkEvt = NULL;
void localPermitJoinState(void){
	static bool assocPermit = 0;
	if(assocPermit != zb_isDeviceJoinedNwk()){
		assocPermit = zb_isDeviceJoinedNwk();
		if(assocPermit){
			brc_winkEvt = TL_ZB_TIMER_SCHEDULE(led_wink,NULL,300);
			my_set_gpio_level(g_config_st.join_net_io,g_config_st.join_net_io_level);
		}else{
			led_off(LED_PERMIT);
			TL_ZB_TIMER_CANCEL(&brc_winkEvt);
			my_set_gpio_level(g_config_st.join_net_io,!g_config_st.join_net_io_level);
		}
	}
}
...
void BTN_SINGLE_Click_Handler(void* btn){
	unsigned short short_addr = g_zbMacPib.shortAddress;
	unsigned char send_data[4];
	send_data[0] = 0xA5;
	send_data[1] = 0x5A;
	send_data[2] = (short_addr>>8)&0xff;
	send_data[3] = short_addr&0xff;
	my_data_broadcast(send_data,4);
}
...
void BTN_DOUBLE_Click_Handler(void* btn){
	...
	send_data[0] = 0x5A;
	send_data[1] = 0xA5;
	send_data[2] = g_config_st.device_id;
	send_data[3] = g_config_st.device_id;
	my_data_broadcast(send_data,4);
}

int periodic_broadcast(void *par){
	unsigned char arrary[4];
	arrary[0] = 0x5A;
	arrary[1] = 0xA5;
	arrary[2] = g_config_st.device_id;
	arrary[3] = g_config_st.device_id;
	my_data_broadcast(arrary,4);
	return 0;
}
static unsigned char last_state = 0;
ev_timer_event_t *brc_broadcastEvt = NULL;
void BTN_LONG_PRESS_HOLD_Handler(void* btn){
	if(btn != &btn3){ // 模式发生了改变
		...
		if(3 == g_config_st.module_function ){
			if(last_state != 3){
				brc_broadcastEvt = TL_ZB_TIMER_SCHEDULE(periodic_broadcast,NULL,1000*g_config_st.per_bro_time);
				last_state = 3;
			}
		}
		else{
			if(last_state == 3){
				TL_ZB_TIMER_CANCEL(&brc_broadcastEvt);
				last_state = g_config_st.module_function;
			}
		}
	}
	else{
...
void app_handler(void){
	/*
	 if(2 == g_config_st.module_function){
		 // my_data_unicast( data_pointer , 0x0 , data_len);
	}
	*/
}
