...
#if (DEBUG_BUS==OLED_PRINT_DEBUG_ENABLE || DEBUG_BUS==UART_PRINT_DEBUG_ENABLE)
__attribute__((used)) int _write(int fd, const unsigned char *buf, int size)
{
    (void) fd;
#if (DEBUG_BUS==OLED_PRINT_DEBUG_ENABLE)
#include "../../Mars_B91_Mother_V2.2/device/oled.h"
    OLED_CLS();
    Printf_OLED(0,0,(unsigned char *)buf,size);
    return size;
#elif (DEBUG_BUS==UART_PRINT_DEBUG_ENABLE)
    int i;
    for(i = 0; i < size; i++){
