...
unsigned int Ymodem_read_data(unsigned char *read_buf,unsigned int max_read){
	...
	while(!clock_time_exceed(stamp,2*1000));// 两个字节最大间隔
	return read_index;
}
...
int init_fs(void){
	...
	SD_Card_init();
    FsDiskSdCard.sectorCount = SD_GetSectorCount();
	tlkfs_addDisk((tlkfs_disk_t*)&FsDiskSdCard,1);
	if(f_mount(&g_fs[1],"1:",1)){ // mount SD card
...
_m_lua_interface lua_interface =
{
	...
	.lua_uart_irq_disable  = core_interrupt_disable,
	.init_fs   = init_fs,
