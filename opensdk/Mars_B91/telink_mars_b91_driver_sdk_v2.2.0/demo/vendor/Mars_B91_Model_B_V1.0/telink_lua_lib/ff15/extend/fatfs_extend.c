...
FRESULT ff_scan_root_lua_files(const TCHAR * path,const char * suffix)
{
	...
	if (res == FR_OK)
	{
		while(1)
		{
			...
			if(g_fileinfo.fsize)
			{
				int fname_len = strlen(g_fileinfo.fname);
				if(suffix[0] == '*'){
					count++;
					printf("%s\r\n",g_fileinfo.fname);
				}
				else if(0 == strncmp(suffix,&g_fileinfo.fname[fname_len-suffix_len],suffix_len)){//12345.lua
					count++;
					printf("%s\r\n",g_fileinfo.fname);
				}
			}
		}
		if(0 == count){printf("null");}
	}
	return res;
}
